...
\begin{tikzpicture}[
  ...
  cap/.style={draw, rounded corners=2pt, minimum width=2.0cm, minimum height=0.8cm, align=center, fill=blue!6},
  exec/.style={draw, rounded corners=2pt, minimum width=1.2cm, minimum height=0.6cm, align=center, fill=green!8},
  >=Latex
]
\node[cap] (search) {\texttt{search}};
\node[cap, right=of search] (sum) {\texttt{summarize}};
\node[cap, right=of sum] (gen) {\texttt{generate\_text}};
\node[cap, right=of gen] (trans) {\texttt{translate}};
\node[cap, right=of trans] (pub) {\texttt{publish}};
